...
6. **KRİTİK — Tier-gating gerçek değil (8 Temmuz 2026'da keşfedildi, hâlâ geçerli):** Pro/Kurumsal'ın vaat ettiği özelliklerin HİÇBİRİ kodda tier'a kilitli değil — `/ws/feed` (WebSocket), `subscription_router.py`'deki `frequency=instant` (anlık keyword alert), `/news/{id}/related` (ilişki grafı) hiçbirinde `tier` kontrolü yok, Free/anonim kullanıcı da erişiyor. Pro/Kurumsal'ın tek gerçek farkı günlük istek limiti (100/2000/sınırsız). ~~"WebSocket canlı akış" frontend'de hiç yok~~ — **20 Temmuz 2026'da yapıldı** (bkz. aşağıdaki ✅ maddesi), tier-gating kısmı hâlâ geçerli. "Ham veri export" hâlâ hiç yazılmamış (kapsam/format bile tanımlı değil), keyword-alert'i User/tier'a bağlamak da hâlâ mimari karar bekliyor (Subscriber email-bazlı, User'dan bağımsız).
