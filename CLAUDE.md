...
- **Test sayısı:** 482 test, hepsi yeşil (backend); frontend `tsc --noEmit` temiz
...
6. ~~**KRİTİK — Tier-gating gerçek değil**~~ — ✅ **20 Temmuz 2026'da tamamlandı** (detay aşağıda): arama sonucu tavanı (Free 10/Pro 50/Enterprise 200), `/api/v1/news/{id}/related` (Pro+), `/ws/feed` (Pro+), `subscription_router.py`'deki `frequency=instant` (Pro+, e-posta→User tier eşlemesiyle) artık gerçekten kilitli. "Ham veri export" hâlâ hiç yazılmamış (kapsam/format bile tanımlı değil, v2.0'da ayrı bir iş).
