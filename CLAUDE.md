...
- **Yeni bir UI/bileşen eklerken mobil/responsive kontrolü SONRAKİ bir iş DEĞİL, teslimatın parçası** (kullanıcı isteği, 18 Eylül 2026) — `body`'de global `overflow-x: hidden` (globals.css) var: taşan bir yatay `flex` satırı (tab bar, link listesi, pill/chip grubu) sayfayı kaydırılabilir yapmaz, içeriği SESSİZCE KIRPAR — kullanıcı görmez, hata da fırlamaz (bkz. BİLİNEN NOTLAR). Her yeni yatay `display:flex` satırına `flexWrap:"wrap"` ya da kendi `overflowX:"auto"`'sunu ekle; keyfi uzunlukta metin taşıyan chip/pill'e maxWidth+ellipsis koy (bkz. `NewsCard.tsx` entity chip deseni). Değişiklik sonrası mobil genişlikte (375-390px) gerçekten kontrol et.
...
- **18 Eylül 2026 — admin sekme çubuğu (`/admin/layout.tsx`) + anasayfa footer linkleri (`page.tsx`) + `TrendingPills` mobilde taşıyordu, aynı kök nedenden** (yukarıdaki KODLAMA KURALLARI'ndaki `overflow-x: hidden` kırpma deseni): admin panelde İletişim/Güvenlik sekmeleri fiilen DOKUNULAMAZ haldeydi (kullanıcı bulgusu). Fix: sekme çubuğu kendi `overflowX:auto`'suna kavuştu, footer + TrendingPills `flexWrap`/maxWidth aldı. Yeni bir yatay satır eklerken bunu tekrar keşfetme.
...
- **Güvenlik günlüğü `security_events` (13 Eyl 2026, PR #135-136):** her auth/admin/access/abuse olayı IP + user-agent + nginx `request_id` ile tek satır (`domain/models/security_event.py`, yazma noktası `adapters/api/security_audit.py::record_security_event` — fail-open, gizli bilgi taşımaz). Yeni bir güvenlikle ilgili olay eklerken ŞEMA DEĞİŞMEZ: `EventType`'a sabit ekle + ilgili handler'a tek satır. Okuma `GET /admin/security-events` + `/admin/security` sayfası (e-posta/IP/olay/pencere filtreleri). 90 gün sonra retention job siler (`SECURITY_EVENTS_RETENTION_DAYS`). Bir dahaki "bu hesabı kim açtı?" sorusu: sayfada e-postayı yaz, `register` satırındaki IP'ye tıkla → o IP'nin tüm olayları.
- **`request_id` uçtan uca:** nginx `$request_id` → `X-Request-ID` header (istemcininki EZİLİR) → `RequestContextMiddleware` (ContextVar) → JSON log `request_id` alanı + `security_events.request_id` + yanıt header'ı. nginx access log satırında `rid=<id> cf=<CF-Ray>`. Bir olayı takip etmek için: `docker logs nexstream_nginx | grep rid=<id>` ve `docker logs nexstream_engine | grep <id>`.
- **slowapi anahtarı artık `client_ip` (X-Real-IP), `get_remote_address` DEĞİL** — uvicorn `--proxy-headers`/`--forwarded-allow-ips` ile başlatılmadığı için `request.client.host` prod'da NGINX'in iç IP'siydi: 13 Eyl 2026'ya kadar TÜM ziyaretçiler her endpoint'te tek rate-limit kovasını paylaşıyordu (bir kötüye kullanan herkesi 429'a düşürebilirdi). Yeni bir yerde istemci IP'si gerekiyorsa `request_context.client_ip` kullan, `request.client.host` KULLANMA.
- **`src/infrastructure/*` ASLA `src/adapters/*` import etmez — ihlali prod'u düşürür (13 Eyl 2026):** `logger.py` request_id için `adapters/api/request_context`'i import edince `fastapi` de geldi; scheduler `Dockerfile.light` (requirements-light, fastapi YOK) ile çalıştığı için `setup_logging` import'unda crash-loop'a girdi (restarts=10, ~8 dk scrape tetiklenmedi). ContextVar artık `infrastructure/logging/request_id.py`'de, regresyon testi `tests/infrastructure/test_logger_no_web_deps.py` (subprocess ile "bu modül fastapi çekiyor mu?"). **Yeni bir paylaşılan modüle import eklerken sor: light image'da (scheduler) o paket var mı?** `requirements-light.txt` ile karşılaştır.
