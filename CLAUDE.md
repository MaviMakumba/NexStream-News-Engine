...
- **Versiyon:** v2.6 🚀 **CANLIDA: https://nexstreamnewsengine.duckdns.org** (son deploy: 31 Ağustos 2026, PR #77-#78 dahil — **deploy artık tam otomatik**, main'e her merge'de GitHub Actions kendi kendine SSM'e bağlanıp redeploy ediyor, bkz. "Branch" notu aşağıda). İlk canlıya çıkış: 29 Temmuz 2026.
- **Test sayısı:** 849 test, hepsi yeşil (backend, 31 Ağu 2026 — bu oturumun 5 düzeltmesi için +13); frontend `tsc --noEmit` + `next build` temiz.
- **31 Ağu 2026'da kullanıcı canlıda iki sorun bildirdi** (haberlerin sürekli aynı kaynaktan arka arkaya gelmesi + RAG'ın "gram altın haftaya nasıl başladı" gibi bir soruda güncel değil bir hafta önceki habere göre cevap vermesi) — kapsamlı SSM canlı diagnostiğiyle (worker log analizi, DB run-length sıralama testi, container içi `hybrid_search`/`answer_question` çağrıları, RSS feed curl doğrulaması) araştırıldı ve TDD ile **5 gerçek düzeltme** yapıldı, ikisi de PR'a alınıp merge+deploy edildi:
  ...
  - **Kalıcı, çözülmeyen bulgu — sıradaki oturumun gündeminde:** throttle düzeltmesi (PR #77/1) bekleme sürelerini kısalttı (430-520s → başlangıçta 73-237s) ama **40 dakikalık gözlemde tekrar eski seviyeye (420-439s) tırmandı** ve proaktif throttle hiç tetiklenmedi (0 kez) — bu, asıl kısıtın TPM değil **TPD (günlük) kota** olduğunu gösteriyor: günlük ~206 haberlik hacim, `openai/gpt-oss-20b`'nin 200K TPD tavanına zaten çok yakın/üstünde. **Bkz. YOL HARİTASI madde 25 — bu, sıradaki oturumun İLK gündem maddesi olarak not düşüldü** (kullanıcı isteği, 31 Ağu 2026).
...
- **27 Ağu 2026'da (aynı gün, yeni oturum) 7. bulgu üzerine çalışıldı ama daha derin bir işe evrildi:** kullanıcı sözlük-tabanlı istisna yerine gerçek bir skorlama istedi, brainstorm "hybrid_search'ün TÜM skor mekanizmasını yeniden tasarlama (sorgu-varlık doğrulaması + credibility fold-in) + görünür bir güven rozeti" işine büyüdü. Ayrıca aynı oturumda: RAG kanıt paketine `content` eklendi (bounded fix — eskiden sadece başlık gidiyordu), test süitinde GERÇEK bir SMTP/Resend sızıntısı bulunup kapatıldı (bkz. BİLİNEN NOTLAR), CLAUDE.md/README/CHANGELOG bölünüp güncellendi (819→650 satır). **Tam plan yazıldı ve onaylandı ama HENÜZ UYGULANMADI** — `feature/arama-skoru-ve-guven-rozeti` dalında (commit'lendi, henüz push edilmedi), spec: `docs/superpowers/specs/2026-08-27-arama-skoru-ve-guven-rozeti-design.md`, plan: `docs/superpowers/plans/2026-08-27-arama-skoru-ve-guven-rozeti.md` (6 task, TDD). **Sıradaki oturumun İLK işi bu planı uygulamak** (subagent-driven mi inline mi kullanıcı henüz seçmedi). Full-article-scraping (madde 18) brainstorm'u bu turun ortasında duraklatıldı — kaldığı yer: kapsam RAG-only/on-demand, cache Redis-TTL kararı verildi, bu plan bittikten SONRA devam edilecek. Spec'in tam 5 senaryolu QA turu da hâlâ yapılmadı.
...
25. **🔴 SIRADAKI OTURUMUN İLK MADDESİ (kullanıcı isteği, 31 Ağu 2026):
    Groq günlük token/hacim maliyetini düşürme.** 31 Ağu 2026'da worker
    starvation bug'ı (bkz. MEVCUT DURUM, PR #78) düzeltildi ama bu sadece
    Groq kısıtının ADİL PAYLAŞILMASINI sağladı — asıl tavanı yükseltmedi.
    Kanıt: proaktif TPM throttle (PR #77) 40 dakikalık canlı gözlemde
    bekleme sürelerini önce kısalttı (430-520s → 73-237s) ama sonra tekrar
    eski seviyeye (420-439s) tırmandı VE hiç tetiklenmedi (0 kez) — yani
    asıl kısıt TPM değil **TPD (günlük) kota**: günlük ~206 haberlik hacim
    zaten `openai/gpt-oss-20b`'nin 200K TPD tavanına çok yakın/üstünde
    (bkz. BİLİNEN NOTLAR "26 Ağu 2026'da 199.555/200.000" notu — aynı
    tıkanıklık, farklı görünüm). **İki olası kol, muhtemelen ikisi birden
    gerekecek:** (a) haber başına token maliyetini düşür — `groq_analyzer.py`
    prompt'unu/`max_tokens=600`'ü küçültmek, (b) günlük analiz HACMİNİ düşür
    — kaynak başına scrape limitini (`BaseRssScraper.limit = 25`) azaltmak
    ya da analiz öncesi ucuz bir ön-filtre (ör. çok kısa/düşük kaliteli
    içerikleri Groq'a hiç göndermemek). Bounded bir iş değil — TDD ile ele
    alınmalı, önce hangi kolun (a/b, ya da ikisi) tercih edileceği kullanıcıyla
    netleştirilmeli.
