...
- **Test sayısı:** 517 test, hepsi yeşil (backend); frontend `tsc --noEmit` temiz
...
7. **Dependabot kurulumu** — `.github/dependabot.yml` (pip + npm ekosistemleri, haftalık tarama) henüz eklenmedi. Şu an bağımlılık güncellemeleri tamamen manuel: yeni sürüm fark edilmeli → `requirements.txt`/`package.json` elle güncellenmeli → venv'e/node_modules'e yüklenmeli → test suite koşturulmalı → Docker image REBUILD edilmeli (sadece `restart` yetmez, pin'ler image'a build anında gömülür). Dependabot bu sürecin "yeni sürüm var mı" + "PR aç" + "CI ile doğrula" kısımlarını otomatikleştirir, review/merge/rebuild kararı kullanıcıda kalır (bilinçli — otomatik merge/deploy istenmedi). Kullanıcı onayladı, henüz uygulanmadı.
...
- **KAPATILMAYANLAR (bilinçli, sonraki oturum):** `/ws/feed` bağlantı sayısı hâlâ sınırsız (tek Pro hesap binlerce bağlantı açabilir); public `/news/search` kota atlatma (Free 100/gün yerine 30/dk = ~43k/gün — endpoint landing demosu için bilinçli public, kısıtlamak ürün kararı); yedekler şifresiz/offsite değil (VPS deploy sırasında yapılmalı); token'lar DB'de düz metin (belgelenmiş sadelik tercihi); `/docs` prod'da açık (yol haritasındaki "API dökümantasyon portalı" ürün özelliğiyle çelişmesin diye BİLİNÇLİ bırakıldı).
