...
- **Test sayısı:** 553 test, hepsi yeşil (backend); frontend `tsc --noEmit` temiz. **Paket ~22 saniye sürüyor** (29 Tem 2026'da 400sn'den düşürüldü — bkz. "v2.0 RAM optimizasyonu" bloğu).
...
**⏭️ SIRADAKİ OTURUM (29 Tem 2026 akşamı planlandı) — owner rolü + gerçek e-posta gönderimi:**
Spec: `docs/superpowers/specs/2026-07-29-owner-rolu-ve-gercek-email-gonderimi-design.md` (commit `e0d68f0`).
**KOD YAZILMADI.** Dört parça: (1) `owner` rolü (`user<moderator<admin<owner`, `OWNER_EMAILS` env
bootstrap, API'den asla atanamaz, migration gerekmez); (2) owner her yerde Enterprise muamelesi —
domain'de saf `effective_tier(tier, is_owner)` + `auth_utils.user_effective_tier(user)` sarmalayıcı
(domain `settings` import edemez), 8 gating noktası, public `/news/search` DEĞİŞMEZ; (3) kademeli rol
yönetimi (hedefin rolü < istek sahibinin rolü VE atanacak rol <= istek sahibinin rolü → admin başka
adminin rolünü düşüremez); (4) `SmtpEmailAdapter` + ortak `_HtmlEmailAdapter` ara sınıfı +
`EMAIL_PROVIDER=auto|smtp|resend|console` + prod'da Console'a düşüşün `logger.error` ve `/health`
ile görünür kılınması. **BLOKE:** kullanıcı Gmail app password'ünü kendisi `.env`'e yazacak
(`SMTP_USER`/`SMTP_PASSWORD`); kullanıcı "önce test et, sonra başla" dedi — gerçek gönderim
`boeingb747.800@gmail.com`'a da düşmeli (Resend'in yapamadığı şey tam olarak bu).
