...
- **Test sayısı:** 702 test, hepsi yeşil (backend, 21 Ağu 2026); frontend `tsc --noEmit` + `next build` temiz. Paket ~22 saniye sürüyor (29 Tem 2026'da 400sn'den düşürüldü — bkz. CHANGELOG "v2.0" bloğu).
...
20. **"Kaynaklar" (story cluster) UI'ının kullanışlılığı sorgulanıyor (21 Ağu 2026,
    ...
    haber listesi göstermek yerine sadece kaynak İSİMLERİNİ (ör. "HT Spor, TRT
    Haber, CNN") göstermenin daha iyi olabileceğini ya da özelliğin tamamen
    kaldırılmasını önerdi ama kesin karar vermedi. Bu oturumda tema ve piyasa
    ticker'ı işine öncelik verildiği için ELE ALINMADI — sıradaki oturumun ilk
    işlerinden biri olmalı: önce "tam çalışmıyor" kısmının somut bir bug mu yoksa
    kullanıcı algısı mı olduğunu netleştir (ör. `get_story_cluster`'ın gerçek
    davranışını canlıda test et), sonra UI yönünü (isim listesi vs. kaldırma vs.
    başka bir tasarım) brainstorming ile netleştir.
