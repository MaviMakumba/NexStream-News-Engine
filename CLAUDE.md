...
- **Versiyon:** v2.9 🚀 **CANLIDA: https://nexstreamnewsengine.duckdns.org** (son deploy: 1 Eylül 2026, PR #90 dahil — deploy tam otomatik, main'e her merge'de GitHub Actions SSM'e bağlanıp redeploy ediyor). İlk canlıya çıkış: 29 Temmuz 2026.
...
- **1 Eylül 2026 (bu oturum) — roadmap #25'in devamı + canlı güvenlik/UX taraması, hepsi merge+deploy edildi:**
  ...
  - **PR #90 — mobil responsive bug (kullanıcı bulgusu, iPhone 12 dikey mod):** `/dashboard`'da ekran sağa kaydırılabiliyor, uzun entity chip'leri karttan taşıyordu. Kök neden: `.badge` class'ı `white-space:nowrap`+`flex-shrink:0` kullanıyor (kısa sabit metinler için doğru) ama entity isimleri keyfi uzunlukta — `NewsCard.tsx`'teki entity chip'lerine (+ İlgili Haberler panelindeki `common_entities`, aynı risk) `maxWidth`+ellipsis eklendi, `html`'e de savunma amaçlı `overflow-x:hidden`. **Canlı tarayıcı doğrulaması yapılamadı** (bkz. BİLİNEN NOTLAR, playwright/Chromium indirme sorunu) — **kullanıcı deploy sonrası telefondan kontrol etmeli.**
