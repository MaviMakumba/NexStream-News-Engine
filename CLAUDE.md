...
- **Test sayısı:** 407 test, hepsi yeşil (backend); frontend `npm run build` temiz
...
**Tamamlananlar:** v1.2 → v1.11 (detaylar yukarıdaki milestone'larda) + v1.11 sonrası 6 ara iş (şifre sıfırlama, prod deploy tutarlılığı, kritik ChromaDB indeksleme bug fix, arama recency sıralaması, retention job, cookie tabanlı session auth — detay MEVCUT DURUM'da). 407 test, lokalde TAM çalışır (billing dahil — `BILLING_DEV_MODE=true` ile Stripe'sız demo). Gerçek Stripe entegrasyonu kod tarafında hazır; sadece gerçek hesap + `STRIPE_*` anahtarları + `stripe listen` webhook'u gerekir (v2.0 deploy işi). **v1.12 başladı** — madde 1'in Navbar kısmı bitti, gerisi sıradaki adım.
...
6. **KRİTİK — Tier-gating gerçek değil (8 Temmuz 2026'da keşfedildi):** Pro/Kurumsal'ın vaat ettiği özelliklerin HİÇBİRİ kodda tier'a kilitli değil — `/ws/feed` (WebSocket), `subscription_router.py`'deki `frequency=instant` (anlık keyword alert), `/news/{id}/related` (ilişki grafı) hiçbirinde `tier` kontrolü yok, Free/anonim kullanıcı da erişiyor. Pro/Kurumsal'ın tek gerçek farkı günlük istek limiti (100/2000/sınırsız). Ayrıca **"WebSocket canlı akış" frontend'de hiç yok** — backend endpoint'i çalışıyor ama Next.js'te ona bağlanan tek satır kod yok (v1.10 Streamlit→Next.js geçişinde düşmüş olmalı, CLAUDE.md'nin v1.7 notu yanıltıcı). "Ham veri export" ise baştan beri hiç yazılmamış (kodda export/CSV/download araması sıfır sonuç verdi). Bu üçü (WS ticker inşası, keyword-alert'i User/tier'a bağlama — Subscriber şu an email-bazlı, User'dan bağımsız, mimari karar gerekiyor — ve export) henüz yapılmadı, sıradaki adım.
